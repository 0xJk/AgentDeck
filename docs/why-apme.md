...
Judge는 모든 run에 돌아간다. 클라우드 API를 쓰면 비용이 폭발하거나 사용자가 judge를 끄게 되고, 끄면 데이터가 축적 안 된다. 그래서 **로컬 MLX (Qwen3.5-30B)가 기본**이고 보조로 OpenClaw Gateway를 쓴다. 둘 다 로컬 프로세스라 전수 평가(`sampleRate: 1.0`)가 가능하다.
...
Judge는 **로컬 MLX (Qwen3.5-30B)** 만 쓴다. 보조로 OpenClaw Gateway가 있지만 그것도 로컬이다. 이유:
