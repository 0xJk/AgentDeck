...
- TTL 로 제거된 Codex row 는 late `codex_tool_end`, `codex_stop`, notify completion 이벤트로 재생성하지 않는다. 재생성은 `codex_user_prompt_submit` / `codex_tool_start` 같은 start-like 이벤트에만 허용한다.
