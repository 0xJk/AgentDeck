...
- AgentDeck connects to the user's local OpenClaw Gateway over WebSocket (`ws://127.0.0.1:18789`). The Gateway itself is the user's own program, started independently. AgentDeck never spawns the `openclaw` CLI and never enumerates `~/.openclaw/`; that path is referenced only as a literal hint string in the user-facing pairing copy (since users running OpenClaw typically know its config file lives there) — the runtime never points `NSOpenPanel.directoryURL` *at* `~/.openclaw/`, never calls `FileManager` against it, and never otherwise programmatically traverses that directory. (The Import-token panel's `directoryURL` is set to the user's real home directory so the dialog opens somewhere familiar — see step 2 below — and the user navigates to `~/.openclaw/` themselves if they want.) No file under that directory is opened unless the user explicitly picks it in step 2.
...
- **Optional shared-token mode.** If the user's Gateway is configured to require a shared token in addition to (or instead of) device pairing, the user can provide it in two ways:
  ...
  2. Tap "Import token" in the OpenClaw troubleshoot row. AgentDeck presents an `NSOpenPanel`. The panel's title and message text mention `~/.openclaw/openclaw.json` as the *typical* OpenClaw config location — this is human-readable hint text only. `directoryURL` is set to the user's real home directory (resolved via `getpwuid(getuid()).pw_dir`, since `NSHomeDirectory()` inside the sandbox returns the app's container path which is not where OpenClaw lives). The panel itself runs in Powerbox outside the sandbox and is allowed to present that path as a navigation starting point; the hint grants no read permission by itself — only what the user explicitly selects in the panel is readable. The panel does **not** preselect any specific file, the user is free to navigate elsewhere or cancel, and AgentDeck never opens, traverses, or enumerates any path under the user's real home outside the file the user picks here. The selected file's bytes are read into memory (`Data(contentsOf:)`) and parsed via `JSONSerialization`; from the resulting object **only the top-level `auth.token` string is used** — it is written to the macOS Keychain via `OpenClawGatewayTokenStore`, and every other parsed value goes out of scope and is freed when the import method returns. The read is performed inside a `startAccessingSecurityScopedResource()` / `defer stop` pair under the existing `com.apple.security.files.user-selected.read-write` entitlement; the URL is not bookmarked, no other path is opened, and no sandbox-external path is hardcoded as a programmatic file-system *read* target — `directoryURL` is a navigation hint to Powerbox, not a `FileManager` access.
